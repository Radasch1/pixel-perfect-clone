      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {presets.map((p, i) => (
          <Reveal key={p.name} delay={(i % 3) * 90} as="article">
            <div className="group flex h-full flex-col overflow-hidden rounded-md border border-border bg-card shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-primary/60 hover:shadow-lift">
              <ScreenshotSlot
                ratio="16 / 10"
                zoomOnHover
